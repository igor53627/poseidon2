...
/**
 * @title Direct Gas Test
 * @notice Simple gas testing without complex dependencies
 */
contract DirectGasTest {
    
    event GasResult(string implementation, uint256 inputSize, uint256 gasUsed, uint256 result);
    ...
    function testZemseGas() public returns (uint256 gasUsed, uint256 result) {
        ...
        emit GasResult("zemse", 3, gasUsed, result);
    ...
    function testCardinalGas() public returns (uint256 gasUsed, uint256 result) {
        ...
        emit GasResult("cardinal", 3, gasUsed, result);
        return (gasUsed, result);
    }
    
    function benchmarkAllSizes() public {
        console.log("=== Poseidon2 Gas Benchmark ===");
        console.log("Implementation | Input Size | Gas Used | Result");
        console.log("---------------|------------|----------|-------");
    ...
    function _benchmarkSize(uint256 size) internal {
        ...
        console.log(
            string(abi.encodePacked(
                "zemse          | ", vm.toString(size), "         | ",
                vm.toString(zemseGas), "     | ",
                vm.toString(Field.toUint256(zemseResult))
            ))
        );
        
        console.log(
            string(abi.encodePacked(
                "cardinal       | ", vm.toString(size), "         | ",
                vm.toString(cardinalGas), "     | ",
                vm.toString(cardinalResult)
            ))
        );
        
        // Calculate savings
        uint256 savings = cardinalGas > zemseGas ? ((cardinalGas - zemseGas) * 100) / cardinalGas : 0;
        console.log("Savings vs Cardinal:", savings, "%");
        console.log("---");
    }
    
    function benchmarkMerkle() public {
        console.log("\\n=== Merkle Tree Benchmark ===");
        console.log("Iteration | Zemse Gas | Result");
        console.log("----------|-----------|-------");
        ...
        for (uint256 i = 0; i < 10; i++) {
            ...
            console.log(
                string(abi.encodePacked(
                    vm.toString(i), "         | ",
                    vm.toString(gasUsed), "     | ",
                    vm.toString(Field.toUint256(result))
                ))
            );
        }
    }
